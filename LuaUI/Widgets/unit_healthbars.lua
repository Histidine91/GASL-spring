...
local barColors = {
  ...
  energy  = { 0.70,0.70,0.00,barAlpha },
  slow    = { 0.50,0.10,0.70,barAlpha },
  suppression = { 1.00,0.10,0.10,barAlpha },
  ...
  function DrawUnitInfos(unitID,unitDefID, ud)
   ...
   --if (not health)    then health=-1   elseif(health<1)    then health=1    end
    ...
    --// BARS //-----------------------------------------------------------------------------
      ...
      --// ENERGY
      local energyState = GetUnitRulesParam(unitID,"energy")
      if energyState and (energyState < 0.75) then
		  AddBar("energy",energyState,"energy",(fullText and floor(energyState*100)..'%') or '')
      end
      ...
      local suppressionState = GetUnitRulesParam(unitID,"suppression")
      if (suppressionState and (suppressionState>0)) then
        AddBar("suppression",suppressionState,"suppression",(fullText and floor(suppressionState*100)..'%') or '')
      end
