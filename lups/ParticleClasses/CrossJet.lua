-- $Id: CrossJet.lua 3171 2008-11-06 09:06:29Z det $
-----------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------

local CrossJet = {}
CrossJet.__index = CrossJet
...
function CrossJet:BeginDrawDistortion()
  glUseShader(jitShader)
    glUniform(timer2Uniform, spGetGameSeconds())
end

function CrossJet:EndDrawDistortion()
  glUseShader(0)
  glTexture(1,false)
  glTexture(2,false)
  lastTexture1,lastTexture2 = "",""
...
function CrossJet:DrawDistortion()
  if (lastTexture1~=self.texture1) then
    glTexture(1,self.texture1)
    lastTexture1=self.texture1
  end
...
function CrossJet:ReInitialize()
  self.dieGameFrame = self.dieGameFrame + self.life
end

function CrossJet.Initialize()
  ...
  if (jetShader == nil) then
    print(PRIO_MAJOR,"LUPS->CrossJet: (color-)shader error: "..gl.GetShaderLog())
    return false
  end
  ...
  if (jitShader == nil) then
    print(PRIO_MAJOR,"LUPS->CrossJet: (jitter-)shader error: "..gl.GetShaderLog())
    return false
  end
...
function CrossJet:CreateParticle()
...
function CrossJet.Create(Options)
  local newObject = MergeTable(Options, CrossJet.Default)
  setmetatable(newObject,CrossJet)  -- make handle lookup
  newObject:CreateParticle()
  return newObject
end

function CrossJet:Destroy()
