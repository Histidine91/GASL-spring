...
--------------------------------------------------------------------------------
  ...
  local function GameFrame(_,n)
	...
	local lus = false
        ...
        local uid = Spring.CreateUnit(cunit.defname,x,y,z,"s", 0, false, false);
...
local function GetBound1(pixels,start,finish,dirX,dirY,length)
  ...
  for i=start, finish, ((start>finish) and -1) or 1 do
    for n=0, length do
      if (not pixels[1+i*dirY+n*dirX]) or (not pixels[1+i*dirY+n*dirX][1+i*dirX+n*dirY]) then
	bound = i
	break
      elseif (pixels[1+i*dirY+n*dirX][1+i*dirX+n*dirY][4]>0) then
        bound=i;
        break;
      end;
    end;
