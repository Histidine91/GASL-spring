...
local SUPPRESSION_ROF_MOD = 1	-- 100% increase in reload time at full suppression
...
function GG.UpdateUnitAttributes(unitID, frame)
	...
	local suppression = spGetUnitRulesParam(unitID,"suppression")
	
	-- SLOW --
	local slowState = spGetUnitRulesParam(unitID,"slowState")
	
	if selfReloadSpeedChange or selfMoveSpeedChange or slowState or selfTurnSpeedChange or suppression then
		...
		local reloadMult = (slowMult)*(1/(1+suppression*SUPPRESSION_ROF_MOD))--*(selfReloadSpeedChange or 1)
